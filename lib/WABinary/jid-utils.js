...
const lidToJid = (lid) => {
    ...
    const decoded = (0, exports.jidDecode)(lid);
    if (!decoded) return lid;
    const device = decoded.device || 0;
    return (0, exports.jidEncode)(decoded.user, 's.whatsapp.net', device || undefined);
};
...
const jidNormalizedUser = (jid) => {
    ...
    const normalized = server === 'c.us' || server === 'lid' ? 's.whatsapp.net' : server;
    return (0, exports.jidEncode)(user, normalized);
};
