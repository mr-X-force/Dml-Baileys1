...
const makeChatsSocket = (config) => {
    ...
    const pinMessage = async (jid, messageKey, type) => {
        ...
        if (isPinning) {
            ...
            itemAttrs.sender = (0, WABinary_1.jidNormalizedUser)(senderJid);
            itemAttrs.type = duration;
        }
        await query({
            tag: 'iq',
            attrs: { to: jid, xmlns: 'w:g:2', type: 'set' },
            content: [{ tag: 'pin', attrs: { v: '2' }, content: [{ tag, attrs: itemAttrs }] }]
        });
    };
    const clearChatMessages = (jid, lastMsg) => {
        return chatModify({ clearChat: { lastMsg: lastMsg || {} } }, jid);
    };
