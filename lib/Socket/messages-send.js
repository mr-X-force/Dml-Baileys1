...
const toxic = require('./groupStatus');
const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        for (let jid of jids) {
            const _decoded = WABinary_1.jidDecode(jid)
            if (_decoded?.server === 'lid') continue
            const user = _decoded?.user
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                let [groupData, senderKeyMap] = await Promise.all([
                    (async () => {
    ...
    const toxicHandler = new toxic(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await toxicHandler.sendStatusWhatsApp(content, jids);
        },
            ...
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        ...
                        case 'EVENT':
                            return await toxicHandler.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await toxicHandler.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await toxicHandler.handleGroupStory(content, jid, quoted)
                    }
                }
