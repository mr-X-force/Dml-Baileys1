...
const makeSocket = (config) => {
    ...
    if (printQRInTerminal) {
      ...
      const syncDisabled = typeof config.shouldSyncHistoryMessage === 'function' && Defaults_1.PROCESSABLE_HISTORY_TYPES.filter(t => t !== undefined).map(syncType => config.shouldSyncHistoryMessage({ syncType })).filter(x => x === false).length === Defaults_1.PROCESSABLE_HISTORY_TYPES.filter(t => t !== undefined).length;
      if (syncDisabled) {
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        */
        if (diff > keepAliveIntervalMs + 3000) {
