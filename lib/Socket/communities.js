  ...
  const makeCommunitiesSocket = (config) => {
      ...
      const communityFetchAllParticipating = async () => {
          ...
          if (communitiesChild) {
              ...
              for (const communityNode of communities) {
                  const meta = (0, exports.extractCommunityMetadata)({ tag: 'result', attrs: {}, content: [communityNode] });
                  data[meta.id] = meta;
              }
          }
      ...
      return {
          ...
          communityUpdateDescription: async (jid, description) => {
              const meta = await communityMetadata(jid);
              const prev = meta.descId ?? null;
