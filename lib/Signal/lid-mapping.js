...
const isLidUser = jid_utils_1.isLidUser;
const jidDecode = jid_utils_1.jidDecode;
const jidNormalizedUser = jid_utils_1.jidNormalizedUser;

const isPnUser = (jid) => typeof jid === 'string' && jid.endsWith('@s.whatsapp.net');
const isHostedPnUser = (jid) => typeof jid === 'string' && jid.endsWith('@hosted');
const WAJIDDomains = { HOSTED_LID: 'hosted.lid' };
...
class LIDMappingStore {
    ...
    async _getLIDsForPNsImpl(pns) {
        ...
        const addResolvedPair = (pn, decoded, lidUser) => {
            ...
            const pnDevice = decoded && decoded.device !== undefined ? decoded.device : 0;
            const deviceSpecificLid = `${normalizedLidUser}${!!pnDevice ? `:${pnDevice}` : ''}@${
                decoded && decoded.server === 'hosted' ? 'hosted.lid' : 'lid'
            }`;
        ...
        if (pending.length) {
            ...
            for (const { pn, pnUser, decoded } of pending) {
                ...
                if (cached && typeof cached === 'string') {
                    ...
                    const device = (decoded && decoded.device) || 0;
    ...
    async _getPNsForLIDsImpl(lids) {
        ...
        const addResolvedPair = (lid, decoded, pnUser) => {
            ...
            const lidDevice = decoded && decoded.device !== undefined ? decoded.device : 0;
            const pnJid = `${pnUser}:${lidDevice}@${
                decoded && decoded.domainType === WAJIDDomains.HOSTED_LID ? 'hosted' : 's.whatsapp.net'
            }`;
