...
class LIDMappingStore {
    ...
    lidToJid(lid) {
        ...
        if (!decoded) return lid;
        const device = decoded.device || 0;
        return `${decoded.user}${device ? `:${device}` : ''}@s.whatsapp.net`;
    }
    ...
    async resolveToJid(jidOrLid) {
        if (!jidOrLid) return jidOrLid;
        if (!jidOrLid.endsWith('@lid')) return jidOrLid;
        const pn = await this.getPNForLID(jidOrLid);
        return pn || this.lidToJid(jidOrLid);
    }

    async resolveToJids(jids) {
        return Promise.all((jids || []).map(j => this.resolveToJid(j)));
    }
            ...
            try {
                ...
                for (const lidUser of missingLids) {
                    ...
                    if (pnUser && typeof pnUser === 'string') {
                        ...
                        const fallbackLid = lids.find(l => l && (0, jid_utils_1.jidDecode)(l)?.user === lidUser);
                        if (fallbackLid) {
                            const decoded = (0, jid_utils_1.jidDecode)(fallbackLid);
                            if (decoded) {
                                const fallbackJid = `${decoded.user}${decoded.device ? `:${decoded.device}` : ''}@s.whatsapp.net`;
                                result.push({ lid: fallbackLid, pn: fallbackJid });
                            }
                        }
                    }
                }
    ...
    async getPNForLID(lid) {
        ...
        const promise = this.getPNsForLIDs([lid]).then(r => r?.[0]?.pn || this.lidToJid(lid));
